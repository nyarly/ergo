...
%% @end
-spec(run([ergo:target()]) -> ergo:command_result() ).
run(Targets) ->
  ergo_events:build_requested(Targets).

%% @spec:	requires(product::ergo:produced(), dependency::ergo:produced()) -> ergo:command_result().
%% @doc:	Declares that the contents of [product] depend on those of [dependency]
%% @end
-spec(requires(ergo:produced(), ergo:produced()) -> ergo:command_result()).
...
%% @end
-spec(provides(ergo:task(), ergo:produced()) -> ergo:command_result()).
provides(Task, Product) ->
  ergo_events:production_noted(Task, Product).

%% @spec:	might_skip(Task::ergo:task()) -> ergo:command_result().
...
-spec(might_skip(ergo:task()) -> ergo:command_result()).
might_skip(Task) ->
  ok.

%% @spec:	skip(task::ergo:task()) -> ergo:command_result().
%% @doc:	The [task] has determined that it need not be run
%% @end
-spec(skip(ergo:task()) -> ergo:command_result()).
skip(Task) ->
  ergo_events:task_skipped(Task).

%% @spec:	together(first::ergo:task(), second::ergo:task()) -> ergo:command_result().
%% @doc:	Whenever [first] is run, [second] should also run.
%% @end
-spec(together(ergo:task(), ergo:task()) -> ergo:command_result()).
together(First, Second) ->
  ergo_events:tasks_joint(First, Second).

%% @spec:	order(first::ergo:task(), second::ergo:task()) -> ergo:command_result().
%% @doc:	When [first] and [second] would both be run,
%%              first precedes second
%% @end
-spec(order(ergo:task(), ergo:task()) -> ergo:command_result()).
order(First, Second) ->
