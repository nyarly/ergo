...
-export([start/0, create_tables/0, file_digest/2, store/4, check/3, elidability/3]).

-include_lib("kernel/include/file.hrl").

-record(ergo_task_cache, {task_name, dependencies = [], products = []}).
-record(ergo_task_elidibility, {key :: {ergo:workspace_name(), ergo:taskname()}}).
...
elidability(Root, Task, false) ->
  {atomic, _} = mnesia:transaction(
    fun() ->
        mnesia:delete(#ergo_task_elidibility{ key={Root,Task}})
    end), ok;
elidability(Root, Task, true) ->
  {atomic, _} = mnesia:transaction(
    fun() ->
        mnesia:write(#ergo_task_elidibility{ key={Root,Task}})
    end), ok.
