...
watch() ->
  watch(erdo_workspace:current()).

-spec(wait_on_build(build_id()) -> command_response()).
wait_on_build(Id) ->
  wait_on_build(erdo_workspace:current(), Id).

-spec(run_build([target()]) -> command_response()).
run_build(Targets) ->
  run_build(erdo_workspace:current(), Targets).

-spec(add_product(task(), product()) -> command_response()).
add_product(Task, Product) ->
  add_product(erdo_workspace:current(), erdo_task:current(), Task, Product).

-spec(add_file_dep(product(), product()) -> command_response()).
add_file_dep(From, To) ->
  add_file_dep(erdo_workspace:current(), erdo_task:current(), From, To).

-spec(add_cotask(task(), task()) -> command_response()).
add_cotask(Task, Also) ->
  add_cotask(erdo_workspace:current(), erdo_task:current(), Task, Also).

-spec(add_task_seq(task(), task()) -> command_response()).
add_task_seq(First, Second) ->
  add_task_seq(erdo_workspace:current(), erdo_task:current(), First, Second).

-spec(this_task_preceeds(task()) -> command_response()).
this_task_preceeds(Other) ->
  add_task_seq(erdo_task:current(), Other).

-spec(this_task_follows(task()) -> command_response()).
this_task_follows(Other) ->
  add_task_seq(Other, erdo_task:current()).

-spec(also_run(task()) -> command_response()).
also_run(Other) ->
  add_cotask(erdo_task:current(), Other).

-spec(run_whenever(task) -> command_response()).
run_whenever(Other) ->
  add_cotask(Other, erdo_task:current()).

-spec(dont_elide() -> ok).
dont_elide() ->
  dont_elide(erdo_task:current()).

-spec(dont_elide(task()) -> ok).
dont_elide(Task) ->
  dont_elide(erdo_workspace:current(), Task).

-spec(skip() -> ok).
skip() ->
  skip(erdo_task:current()).

-spec(skip(task()) -> ok).
skip(Task) ->
  skip(erdo_workspace:current(), Task).
