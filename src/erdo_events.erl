...
%% @end
-spec(build_requested(erdo:workspace_name(), [erdo:target()]) -> ok).
...
%% @end
-spec(requirement_noted(erdo:workspace_name(), erdo:produced(), erdo:produced()) -> ok).
...
%% @end
-spec(production_noted(erdo:workspace_name(), erdo:task(), erdo:produced()) -> ok).
...
-spec(task_completed(erdo:workspace_name(), erdo:task()) -> ok).
...
%% @end
-spec(tasks_joint(erdo:workspace_name(), erdo:task(), erdo:task()) -> ok).
...
%% @end
-spec(tasks_ordered(erdo:workspace_name(), erdo:task(), erdo:task()) -> ok).
tasks_ordered(Workspace, First, Second) ->
  send_event(Workspace, {tasks_ordered, {First, Second}}).


-spec(send_event(erdo:workspace_name(), term()) -> ok).
send_event(Workspace, Event) ->
  gen_event:notify({via, erdo_workspace_registry, {Workspace, events, only}}, Event).
